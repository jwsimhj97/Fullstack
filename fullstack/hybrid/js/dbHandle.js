...
// 데이터베이스 생성 및 오픈
function openDB(){
    // window.openDatabase('데이터베이스명', '데이터베이스 버전', '데이터베이스 설명', 용랑(숫자자료형 = 바이트 단위));
    // 5MB == 5*1024*1024
    // db변수(객체)가 오라클, MariaDB의 connection객체라고 생각하면 됨.
    db = window.openDatabase('bookDB', '1.0', '북DB', 5*1024*1024);
    console.logt('1_DB 생성...');
}

// 테이블 생성 트랜잭션 실행
function createTable(){
    // id integer primary key autoincrement 자동증가기능
    // db.transaction(function(ps){...실행문...}); => 오라클, mariaDB의 jdbc에서 ps객체생성문과 유사한 기능
    db.transaction(function(ps){
        var createSQL = 'creat table if not exists book(bookid integer primary key autoincrement, type text, name text)';
